...
また、**GPT-5.1 Auto** は各クエリを最も適したモデルへ自動で振り分けるため、多くの場合ユーザーがモデルを手動で選択する必要はありません。

**GPT-5.1 Auto** はいい感じに最適な Model を選定してくれるわけですね。

GPT-5.1 の包括的な安全対策は、以前の **GPT-5 System Card** で説明したものとほぼ同じとのことでした。
GPT-5 System Card の説明は以下をご参照ください。
...
より AI が人間に寄り添うようになってきたことを感じさせますね。
...
### **Table 3: Image input evaluations（not_unsafe 値、値が高いほど安全）**
...
- OpenAI はこの結果を受け、**画像入力に対する安全性のさらなる改善**を進めています。

## 3. Preparedness Framework（備えのための枠組み）
...
これは、推論・分析・会話型インテリジェンスの次なる世代を担うモデルです。
