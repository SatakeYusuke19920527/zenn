...
**GPT-6 Astraは、安全性が改善した一方で、思考過程から問題行動を見抜きにくくなったモデルです。**

「賢くなったから、安心して全部任せられる」とは限りません。OpenAIのシステムカードで注目したいのは、ベンチマークの点数だけでなく、**エージェントに何を任せ、どこで止めるか**という設計に関わる変化です。

この記事では、全118ページの[GPT-6 Astra System Card](https://deploymentsafety.openai.com/gpt-6-astra/gpt-6-astra.pdf)から、開発者が押さえたい結果を表中心に整理します。全文訳ではなく、重要な結果と限界に絞った非公式の解説です。

原資料の表紙は2026年9月3日付、変更履歴は9月9日付です。以下の評価・製品仕様は、その資料に記載された時点の情報です。

## 目次

- [まず押さえたい三つの変化](#まず押さえたい三つの変化)
- [安全性は「拒否の多さ」では測れない](#安全性は「拒否の多さ」では測れない)
- [外部の指示には強くなった。それでも権限は絞る](#外部の指示には強くなった。それでも権限は絞る)
- [タスクを完了するより、制約を守れるか](#タスクを完了するより、制約を守れるか)
- [最も注意したいのは、思考から監視しにくくなったこと](#最も注意したいのは、思考から監視しにくくなったこと)
- [「Critical」は、何の危険度なのか](#「critical」は、何の危険度なのか)
- [精度の改善も、評価条件とセットで読む](#精度の改善も、評価条件とセットで読む)
- [APIによって、監視できる範囲も違う](#apiによって、監視できる範囲も違う)
- [開発・運用で変えるべきこと](#開発・運用で変えるべきこと)
- [まとめ](#まとめ)
- [出典と詳しく読みたい人へ](#出典と詳しく読みたい人へ)
...
出典：原資料「Monitorability」。監視可能性の低下と、敵対的条件での評価を扱っています。
...
出典：原資料「Preparedness」「Safeguards」。
...
出典：原資料「Hallucinations」「Health」、図6・表6。
...
出典：原資料「Realtime Model Safeguards」「Misalignment Monitoring」。
...
GPT-6 Astraのシステムカードは、「高性能なモデルが出た」という報告だけではありません。エージェントを実際の仕事に使うとき、何をモデルに任せ、何をシステム側で制御すべきかを考える資料でもあります。
...
一次資料：[OpenAI — GPT-6 Astra System Card（全118ページ）](https://deploymentsafety.openai.com/gpt-6-astra/gpt-6-astra.pdf)

| 詳しく知りたいこと | 原資料で読む章 |
| --- | --- |
| 安全な応答と過剰な拒否のバランス | Model Safety |
| 外部からの攻撃的な指示への耐性 | Robustness / Prompt Injection |
| 制約の遵守、欺瞞、評価中だと気づく問題 | Alignment |
| CoT監視、監視回避、サンドバッギング | Monitorability |
| 高度な能力の評価と、それに対応する制御 | Preparedness / Safeguards |

本記事では、読みやすさを優先し、全ベンチマークの網羅、細かな実験手順、グラフの座標列、モデル発言の長い引用は省略しています。数値は原資料の本文・表・図に明記されたものを採用し、設計上の提案は評価結果と区別しました。
