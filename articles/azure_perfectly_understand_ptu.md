---
title: "【PTU徹底解説】Azure OpenAIを高パフォーマンス&安定稼働で使う方法知ってる？【割引有り】"
emoji: "💴"
...
**PTU**で処理能力を事前に予約することで、応答速度の向上と安定稼働を実現することが出来ます。
...
要するに、**PTU**を使うことで、AOAIのアプリケーションを安定稼働させることが出来、コストも削減出来るかもしれないということですね。
...
PTUとはAzure OpenAIのデプロイ方式で4種類ある中の1つであり、占有的なコンピューティングリソースを予約できる契約モデルであることが理解出来ればOKです。
...
#### PTUの種類
PTUの種類は**予約モデル**と**予約なし**に分けることが出来ます。
...
事前にまとめて買ってくれるなら、PTUのHourlyより82%~84%ぐらいお得ですよ〜という認識でOKです。ちなみにPTUは毎月変更可能なので、利用量増えたからPTUも増やすとかも対応可能です。
...
1. PTUはAzure OpenAIサービスで使用されるデプロイ方法で、指定されたスループット（一定時間内に処理できるリクエストの量）を予約する形式です。(Azure VMで言うところのリザーブドインスタンスみたいな感じ)
2. PTUが使用出来るモデルとリージョンは、リージョン毎にPTUが使えないモデルもあるので、契約時点ではよく確認してから契約する必要があります。
3. PTUがどれだけ必要かどうかは、Azure OpenAI Studio の Capacity Plannerで簡単に測定することが可能です。
4.  2024年8月の最新Updateで、クォータの管理が楽になったことやセルフでクォータ要求できる様になったこと、既定のクォータがDeploy時点から付与されていること、コミットメント単位・時間単位の予約支払いモデルがあらたに導入されたことがわかりました。
