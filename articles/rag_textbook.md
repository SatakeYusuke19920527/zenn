...
- [RAG の精度改善の進め方](#rag-の精度改善の進め方)
    - [RAG アプリ全体プロセス](#rag-アプリ全体プロセス)
- [Store の精度向上 (データ準備)](#store-の精度向上-データ準備)
  - [非構造化データのテキスト化](#非構造化データのテキスト化)
  - [チャンク戦略](#チャンク戦略)
    - [チャンク化の効果を検証 (Recall@50)](#チャンク化の効果を検証-recall50)
      - [文書を「ベクトル」で扱う vs. チャンク化](#文書をベクトルで扱う-vs-チャンク化)
      - [チャンクサイズを大きくするとどうなる？](#チャンクサイズを大きくするとどうなる)
      - [境界戦略とオーバーラップの効果](#境界戦略とオーバーラップの効果)
      - [実装で使える Tips](#実装で使える-tips)
  - [フィルタリング](#フィルタリング)
    - [カテゴリ＋フィルタリングとは？](#カテゴリフィルタリングとは)
    - [ざっくり説明すると...](#ざっくり説明すると)
    - [フィルタリングの効果](#フィルタリングの効果)
    - [フォールバック戦略 と メタフィールド設計](#フォールバック戦略-と-メタフィールド設計)
      - [フォールバック戦略 (Fallback Strategy)](#フォールバック戦略-fallback-strategy)
      - [実装ポイント](#実装ポイント)
      - [メタフィールド設計 (Metadata Field Design)](#メタフィールド設計-metadata-field-design)
      - [設計フロー](#設計フロー)
    - [使いどころ \& ベストプラクティス](#使いどころ--ベストプラクティス)
- [Retrieve の精度向上 (検索)](#retrieve-の精度向上-検索)
  - [フルテキスト検索](#フルテキスト検索)
  - [ベクトル検索](#ベクトル検索)
    - [ベクトル検索のチューニング](#ベクトル検索のチューニング)
...
- [Generation の精度向上 (生成)](#generation-の精度向上-生成)
  - [LLM のモデルの選択について](#llm-のモデルの選択について)
  - [LLN の特性と使い分け - “Inference”モデルと“Reasoning”モデル](#lln-の特性と使い分け---inferenceモデルとreasoningモデル)
  - [選択ガイドライン](#選択ガイドライン)
- [Evaluate (評価)](#evaluate-評価)
  - [検索精度](#検索精度)
  - [回答精度](#回答精度)
- [RAG と Fine-Tuning](#rag-と-fine-tuning)
  - [RAFT (Retrieval Augmented Fine Tuning) という手法](#raft-retrieval-augmented-fine-tuning-という手法)
- [RAG と CAG](#rag-と-cag)
  - [CAG ( Cache-Augmented Generation )とは](#cag--cache-augmented-generation-とは)
- [その他](#その他)
- [まとめ](#まとめ)
- [参考文献](#参考文献)
...
RAGの相談は以下、Xから DM いただければ、私がわかる範囲でお答えしますので、お気軽にご連絡ください。
