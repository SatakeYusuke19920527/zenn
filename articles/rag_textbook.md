...
| パターン               | メリット             | デメリット           |
| ---------------------- | -------------------- | -------------------- |
| **LLM による動的分類** | 新規カテゴリにも柔軟 | 毎回推論コストが発生 |
| **正規表現 / ルール**  | 高速・コストゼロ     | ルール保守が必要     |
| **メニュー選択 (UI)**  | クラシックな UX      | ユーザ負荷が高い     |
...
まとめ
カテゴリ＋フィルタリング = 安く早く精度を上げる王道テク
LLM で クエリの意図を分類 → メタデータで前段フィルタ が基本形
実運用では フォールバック戦略 と メタフィールド設計 が成功のカギ
