...
flowchart TB
    %% 中央コア
    C["<b>AIエージェント<br>の要素</b>"]

    %% 周辺ノード
    COORD(["<b>マルチエージェント協調</b><br>複数のエージェントが協力してタスクを解決する。"])
    LLM(["<b>LLM</b><br>エージェントの推論とテキスト生成の中核コンポーネント。"])
    MEMORY(["<b>メモリ</b><br>知識と情報を保存するための短期および長期メモリ。"])
    PLAN(["<b>計画</b><br>タスクを整理し、実行順序を決定するプロセス。"])
    TOOLS(["<b>ツール活用</b><br>外部ツールを使用して LLM の能力を強化する。"])
