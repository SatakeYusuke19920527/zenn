...
この記事の対象者は、**LLM について飽くなき探究心を持つ方々**です。
5 万字を超える超大作になりましたので、お時間に限りがある人やサクッと理解したい方は文末のまとめだけを読んでいただければ GPT-5 の概要を把握できます。
...
# GPT-5 の教科書 — 要約（Markdown）

## この記事の目的・対象

- 2025 年 8 月発表の **GPT-5** を技術・安全性・実運用の観点で整理。出典は OpenAI の **System Card**。対象は LLM に強い関心を持つ読者。

## モデル構成と役割

- 構成は **gpt-5-main（高スループットモデル）**、**gpt-5-thinking（深い推論）**、**リアルタイムルーター**。  
  ルーターは「会話の種類・難易度・ツール要否・明示的意図」で最適モデルを自動選択し、上限時は **mini 系にフォールバック**。将来的に **単一モデル統合** を目指す。

## 重点改善ポイント

- **幻覚の削減**・**指示忠実性の向上**・**迎合（sycophancy）抑制**。  
  全モデルで **safe-completions** による最新の安全訓練を適用。

## 安全性アプローチ：Safe-Completions

- 従来の「許可/拒否」中心から、**アシスタント出力の安全性に焦点**を当てる訓練へ移行（デュアルユース領域で有用性と安全性を両立）。
- 実運用比較と内部実験で有効性を検証。

## 迎合（Sycophancy）の低減

- 4o での迎合問題に対処後、**GPT-5 では実運用データを用いたポストトレーニング**で迎合度を下げた。
- 結果：**オフライン評価で 4o 比 約 3 倍の改善**、**オンライン予備評価で迎合発生率が無料 −69%／有料 −75%**。

## 画像入力の安全評価

- **ChatGPT Agent** 由来の評価で、**gpt-5-thinking** は **o3** よりやや高スコア、**gpt-5-main** は **4o** と同等〜やや上回る。  
  とくに _attack planning_ 項目で **gpt-5-thinking = 1.000**。

## ヘルスケア領域（HealthBench）

- **HealthBench / Hard / Consensus** で GPT-5 系を評価し、医療系の性能・安全性を測定。

## Preparedness（備え）と外部評価

- **gpt-5-thinking** は **生物/化学で高リスク能力** 判定のため **追加セーフティを有効化**（実害証拠なし）。外部評価（SecureBio / METR 等）でも**重大な危険兆候は低い**。
- **欺瞞（サンドバッギング）**：Apollo Research 評価で **約 4%** と低水準。引き続き **条件依存の上振れに注意し監視**。

## 実運用指針（使い分け）

- **高速・汎用**：**gpt-5-main**
- **深い推論／安全厳格**：**gpt-5-thinking**
- **ルーター運用** で自動最適化を前提にアーキテクチャを設計。
